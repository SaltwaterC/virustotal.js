...
/**
 * Wraps all the HTTP requests with the JSON handler
 *
 * @private
 */
var httpWrapper = function(method, options, callback) {
	if (!options.headers) {
		options.headers = {};
	}
	...
	for (key in httpHeaders) {
		if (httpHeaders.hasOwnProperty(key)) {
			options.headers[key] = httpHeaders[key];
		}
	}

	options.headers['user-agent'] = userAgent;

	http[method](options, function(err, res) {
		if (err) {
			callback(err);
			return;
		}
 ...
 * @private
 */
var postRequestMultipart = function(resource, options, callback) {
	if (!callback) {
		callback = options;
		options = {};
	}
	...
	if (options.notify_url) {
		form.append('notify_url', options.notify_url);
	}

	if (options.notify_changes_only) {
		form.append('notify_changes_only', options.notify_changes_only);
	}
	/*jshint camelcase: true */
 ...
 * @private
 */
var checkArgs = function(resource, options, callback, method) {
	if (!method) {
		method = callback;
		callback = options;
	}
	...
	if (!resource) {
		throw new Error(util.format('You need to define a resource argument in order to use %s().', method));
	}

	if (!callback || typeof callback !== 'function') {
		throw new Error(util.format('You need to define a callback for %s().', method));
	}
...
/**
 * Checks for options that should be a positive integer value
 *
 * @private
 */
var checkAbsIntValue = function(value, name, method) {
	value = absInt(value);
	if (value === 0) {
		throw new Error(util.format('The %s option for %s is expected to be a positive integer value.', name, method));
	}
	return value;
};
...
/**
 * Sets some custom HTTP headers for the VT API request
 *
 * @param {Object} headers An object containing the HTTP headers
 *
 * @example
// a common use case would be setting the connection: close request header
// the keep alives may keep the client open after the request is finished
virustotal.setHttpHeaders({
	connection: 'close'
});
 */
exports.setHttpHeaders = function(headers) {
	httpHeaders = headers;
};
 ...
 * @param {Object} options - optional; an Object containing the Private API parameters notify_url and notify_changes_only
 ...
 */
exports.scanFile = function(resource, options, callback) {
	resource = checkArgs(resource, options, callback, 'scanFile');
	postRequestMultipart(resource, options, callback);
};
 ...
 * @param {Object} options - optional; an Object containing the Private API parameters date, 
 * @param {module:virustotal~callback} callback Completion callback
 *
 ...
 */
exports.rescanFile = function(resource, options, callback) {
	resource = checkArgs(resource, options, callback, 'rescanFile');
	...
	if (options.date) {
		options.date = String(options.date);
		if (options.date.length !== 14 || options.date.match(/^\d{14}$/) === null) {
			throw new Error('The date option for rescanFile is expected to be in the %Y%m%d%H%M%S format (example: 20120725170000).');
		}
		arg.date = options.date;
	}

	if (options.period) {
		arg.period = checkAbsIntValue(options.period, 'period', 'rescanFile');
	}

	if (options.repeat) {
		arg.repeat = checkAbsIntValue(options.repeat, 'repeat', 'rescanFile');
	}

	/*jshint camelcase: false */
	if (options.notify_url) {
		options.notify_url = String(options.notify_url);
		arg.notify_url = options.notify_url;
	}

	if (options.notify_changes_only) {
		options.notify_changes_only = String(options.notify_changes_only);
		arg.notify_changes_only = options.notify_changes_only;
	}
	/*jshint camelcase: true */

	postRequest('file/rescan', arg, callback);
};
